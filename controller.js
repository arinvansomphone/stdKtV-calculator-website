...
export function calculateTwice() {
    ...
    if (removalRate < 13) {
        ...
        let timeOutputTwice = document.getElementById("timeOutputTwice");
        if (timeOutputTwice) {
            timeOutputTwice.textContent = `${t_target13}` + " min";
        }

        let UF_RateTwice = document.getElementById("UF_RateTwice");
        if (UF_RateTwice) {
            UF_RateTwice.textContent = '13.0 mL/kg/hr';
        }

        let spKtV_13 = (Keff * (t_target13 + 30)) / (ureaVolume * 1000);
        let newspKtVTwice = document.getElementById("newspKtVTwice");
        if (newspKtVTwice) {
            newspKtVTwice.innerHTML = `<strong>New spKt/V: </strong> ${spKtV_13.toFixed(1)}`;
        }

        let UF_WarningTwice = document.getElementById("UF_WarningTwice");
        if (UF_WarningTwice) {
            UF_WarningTwice.textContent = "NOTE🚨: The target stdKt/V can be achieved using smaller treatment time of " + `${t_target}` + " minutes and a " + `${removalRate.toFixed(1)}` + " mL/kg/hr UF rate.";
        }
...
export function calculateThrice() {
    ...
    if (removalRate < 13) {
        ...
        let timeOutputThrice = document.getElementById("timeOutputThrice");
        if (timeOutputThrice) {
            timeOutputThrice.textContent = `${t_target13}` + " min";
        }

        let UF_RateThrice = document.getElementById("UF_RateThrice");
        if (UF_RateThrice) {
            UF_RateThrice.textContent = '13.0 mL/kg/hr';
        }

        let spKtV_13 = (Keff * (t_target13 + 30)) / (ureaVolume * 1000);
        let newspKtVThrice = document.getElementById("newspKtVThrice");
        if (newspKtVThrice) {
            newspKtVThrice.innerHTML = `<strong>New spKt/V: </strong> ${spKtV_13.toFixed(1)}`;
        }

        let UF_WarningThrice = document.getElementById("UF_WarningThrice");
        if (UF_WarningThrice) {
            UF_WarningThrice.textContent = "NOTE🚨: The target stdKt/V can be achieved using smaller treatment time of " + `${t_target}` + " minutes, a " + `${removalRate.toFixed(1)}` + " mL/kg/hr UF rate, and the current spKt/V.";
        }
